...
import type { Skillset, Power } from './types.js';
import jjkSkillset from './jjk.js';
import narutoSkillset from './naruto.js';
import demonslayerSkillset from './demonslayer.js';
import onepieceSkillset from './onepiece.js';

// Combine all powers from all skillsets with source tags
const allPowers: Power[] = [
    // JJK Powers (tagged)
    ...jjkSkillset.powers.map(p => ({ ...p, char: `${p.char} [JJK]` })),
    
    // Naruto Powers (tagged)
    ...narutoSkillset.powers.map(p => ({ ...p, char: `${p.char} [Naruto]` })),
    
    // Demon Slayer Powers (tagged)
    ...demonslayerSkillset.powers.map(p => ({ ...p, char: `${p.char} [DS]` })),
    
    // One Piece Powers (tagged)
    ...onepieceSkillset.powers.map(p => ({ ...p, char: `${p.char} [OP]` })),
];

// Ultimate Crossover-exclusive moves
const ultimateMoves: Power[] = [
...
const allSummonNames: Record<string, string[]> = {
    ...jjkSkillset.summonNames,
    ...narutoSkillset.summonNames,
    ...demonslayerSkillset.summonNames,
    ...onepieceSkillset.summonNames,
...
const crossoverSkillset: Skillset = {
    name: 'crossover',
...
export default crossoverSkillset;
