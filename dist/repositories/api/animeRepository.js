...
class AnimeRepository {
    ...
    async getUserFavourites(userId) {
        ...
        try {
            const rows = await postgres_js_1.default.getMany(`SELECT anime_id, anime_title, COALESCE(source, 'anilist') as source FROM anime_favourites WHERE user_id = $1 ORDER BY created_at DESC`, [userId]);
    ...
    async isFavourited(userId, animeId) {
        await this._initialize();
        try {
            const row = await postgres_js_1.default.getOne(`SELECT 1 FROM anime_favourites WHERE user_id = $1 AND anime_id = $2`, [userId, animeId]);
            return !!row;
        }
        catch (error) {
    ...
    async isNotifyEnabled(userId, animeId) {
        await this._initialize();
        try {
            const row = await postgres_js_1.default.getOne(`SELECT notify FROM anime_notifications WHERE user_id = $1 AND anime_id = $2`, [userId, animeId]);
    ...
    async addFavourite(userId, animeId, animeTitle, source = 'anilist') {
        await this._initialize();
        try {
            const result = await postgres_js_1.default.query(`INSERT INTO anime_favourites (user_id, anime_id, anime_title, source) 
                 VALUES ($1, $2, $3, $4) 
    ...
    async removeFavourite(userId, animeId) {
        await this._initialize();
        try {
            await postgres_js_1.default.transaction(async (client) => {
    ...
    async enableNotify(userId, animeId) {
        await this._initialize();
        try {
            await postgres_js_1.default.query(`INSERT INTO anime_notifications (user_id, anime_id, notify) 
                 VALUES ($1, $2, true) 
    ...
    async getEnabledNotifications() {
        await this._initialize();
        try {
            const rows = await postgres_js_1.default.getMany(`SELECT user_id, anime_id FROM anime_notifications WHERE notify = true`);
            return rows || [];
        }
        catch (error) {
