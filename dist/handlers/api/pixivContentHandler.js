...
async function createContentEmbed(item, options = {}) {
    ...
    const { resultIndex = 0, totalResults = 1, searchPage = 1, cacheKey = '', contentType = 'illust', hasNextPage = false, shouldTranslate = false, originalQuery = '', mangaPageIndex = 0, sortMode = 'popular' } = options;
    const embed = new discord_js_1.EmbedBuilder().setColor(0x0096FA);
    const rows = [];
    ...
    const resultNavRow = new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.ButtonBuilder()
        .setLabel('◀ Prev')
    ...
    if (contentType !== 'novel' && item.page_count > 1) {
        const pageNavRow = new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.ButtonBuilder()
            .setLabel('◀ Prev Image')
            .setCustomId(`pixiv_pagedown_${cacheKey}`)
            .setStyle(discord_js_1.ButtonStyle.Secondary)
            .setDisabled(mangaPageIndex === 0), new discord_js_1.ButtonBuilder()
    ...
    const searchPageRow = new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.ButtonBuilder()
        .setLabel('⏪ Prev Page')
...
async function buildNovelEmbed(embed, item, options) {
...
async function buildIllustEmbed(embed, item, options) {
    const { sortModeText, nsfwStatus, aiStatus, searchPage, resultIndex, totalResults, mangaPageIndex = 0, shouldTranslate, originalQuery, views, bookmarks, bookmarkRate } = options;
    ...
    try {
        ...
        embed
            ...
            .setDescription(`**Artist:** [${item.user.name}](https://www.pixiv.net/users/${item.user.id})\n` +
...
function createNoResultsEmbed(query, translatedQuery, shouldTranslate, contentType) {
    const embed = new discord_js_1.EmbedBuilder()
        ...
        .setDescription(`No ${contentType === 'novel' ? 'novels' : 'artwork'} found for: **${translatedQuery}**` +
        (shouldTranslate ? `\n(Translated from: "${query}")` : ''))
        .addFields({
        name: '📝 Search Tips',
        value: '• Try Japanese tags (e.g., `巫女` instead of `miko`)\n' +
...
//# sourceMappingURL=pixivContentHandler.js.map