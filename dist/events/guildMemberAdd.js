...
const index_js_2 = __importDefault(require("../config/features/moderation/index.js"));
const index_js_3 = require("../repositories/moderation/index.js");
...
class GuildMemberAddEvent extends BaseEvent_js_1.BaseEvent {
    ...
    async _handleAntiRaid(client, member) {
        try {
            const result = await AntiRaidService_js_1.default.trackJoin(member);
            // If raid detected and not already in raid mode, activate
            if (result.isRaid && !(await AntiRaidService_js_1.default.isRaidModeActive(member.guild.id))) {
                await AntiRaidService_js_1.default.activateRaidMode(member.guild.id, 'system', `Auto-detected: ${result.triggers.join(', ')}`);
            ...
            if (result.isSuspicious && result.triggers.includes('raid_mode_active')) {
                // Check account age
                const ageCheck = AntiRaidService_js_1.default.checkAccountAge(member);
    ...
    async _notifyRaidDetected(client, guild, result) {
        try {
            const settings = await index_js_3.ModLogRepository.get(guild.id);
            ...
            const embed = new discord_js_1.EmbedBuilder()
                .setColor(index_js_2.default.COLORS?.RAID || 0xFF0000)
        ...
        switch (action) {
            case 'kick':
                try {
                    ...
                    AntiRaidService_js_1.default.updateStats(member.guild.id, 'kick');
                }
                catch {
                    // Failed to kick, just flag
                    AntiRaidService_js_1.default.updateStats(member.guild.id, 'flag');
                }
                break;
            case 'ban':
                try {
                    ...
                    AntiRaidService_js_1.default.updateStats(member.guild.id, 'ban');
                }
                catch {
                    AntiRaidService_js_1.default.updateStats(member.guild.id, 'flag');
                }
