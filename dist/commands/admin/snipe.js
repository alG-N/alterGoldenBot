...
function createSnipeEmbed(msg, index, total) {
    const embed = new discord_js_1.EmbedBuilder()
        .setColor(constants_js_1.COLORS.SNIPE || 0x9B59B6)
        .setAuthor({
    ...
    if (msg.attachments && msg.attachments.length > 0) {
        ...
        const imageAttachment = msg.attachments.find(a => a.type?.startsWith('image/') ||
...
class SnipeCommand extends BaseCommand_js_1.BaseCommand {
    constructor() {
        super({
    ...
    async run(interaction) {
        ...
        try {
            ...
            try {
                ...
                if (targetUser) {
                    messages = snipeService.getDeletedMessagesByUser?.(interaction.guild.id, targetUser.id, effectiveCount) || [];
                }
                else {
                    messages = snipeService.getDeletedMessages?.(interaction.guild.id, effectiveCount, targetChannel.id) || [];
                }
            }
            catch (e) {
            ...
            await this.errorReply(interaction, `Failed to retrieve deleted messages: ${error.message}`);
        }
    }
}
exports.default = new SnipeCommand();
